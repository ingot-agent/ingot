...
func TestRuntimeValidatesAndSnapshotsDefinitions(t *testing.T) {
	...
	if err != nil || result.Content != "ok" || fake.calls != 1 {
...
func TestRuntimeInterceptorOrderAndLimits(t *testing.T) {
	...
	events := []string{}
	exports, _, err := New(context.Background(), Config{MaxResultBytes: 5}, Dependencies{Tools: []tool.Tool{fake}, Interceptors: []tool.Interceptor{recordingInterceptor{name: "outer", events: &events}, recordingInterceptor{name: "inner", events: &events}}})
	if err != nil {
		t.Fatal(err)
	}
