...
- 子进程环境从空集合开始，只加入 `environment` 和 `inherit_env` allowlist；
- environment key 重复、非法或 `inherit_env` 中变量不存在时返回 Config Error；
...
stdout/stderr 分别捕获，最终按固定顺序呈现。`max_output_bytes` 在构造采集器时固定分配为 stdout 配额 `ceil(limit/2)` 和 stderr 配额 `floor(limit/2)`；任一流超过自己的配额时截断并显式添加 truncation marker。采集器继续 drain 两条 pipe，不得静默丢弃。无 exit code 的启动或 Context 错误直接返回 error。

## 5. 执行与生命周期

- 使用 `exec.CommandContext` 的等价平台实现，并终止平台 containment primitive 内的进程；脱离 process group 的 Unix daemon 不在 v0.1 强保证范围内；
- effective Context deadline 是 caller deadline 与 per-call/config timeout 的较早者；
- timeout/cancel 后先发 cooperative termination，短 grace period 后强制终止 containment primitive；
...
待确认：官方支持的 shell 列表、Windows process tree 实现、non-zero exit 是否需要额外结构化字段。v0.1 不允许 `shell="auto"`，避免不同主机产生隐式行为差异。
