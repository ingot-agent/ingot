  ...
  while (stack.length) {
    ...
    Object.keys(src2).forEach((key) => {
      ...
      if (isArray(value)) {
        ...
        if (!isObject(des2[key]) || isArray(des2[key])) {
          des2[key] = create();
        }
        stack.push({ src: value, des: des2[key] });
      } else {
        des2[key] = value;
      }
    });
...
function fallbackWithSimple(ctx, fallback, start) {
  return [.../* @__PURE__ */ new Set([
    start,
    ...isArray(fallback) ? fallback : isObject(fallback) ? Object.keys(fallback).filter((locale) => locale !== "default") : isString(fallback) ? [fallback] : [start]
  ])];
}
